...
// NewGraphVisitor creates a GraphVisitor from a simplification result.
// This is the same constructor used internally by ExtractSSAGraphData.
func NewGraphVisitor(result *SSASimplificationResult, packagePrefixFilter func(string) bool, projectPath string) *GraphVisitor {
	return &GraphVisitor{
		BaseSSAVisitor:          BaseSSAVisitor{},
		SSASimplificationResult: result,
...
func ExtractSSAGraphData(simplificationResult *SSASimplificationResult, packagePrefixes []string, projectPath string) SSAGraphData {
	visitor := NewGraphVisitor(simplificationResult, PackageMatcher(packagePrefixes), projectPath)
